...
void User::update(float dt, float elapsedTime, JointParameters const& jointParameters)
{
	...
	float framerateFactor = dt/0.0333;
	for (int h=0; h<2; ++h)
	{
//		float	mouseX = float(app::AppBasic::get()->getMousePos().x)/app::getWindowWidth();
//		float	mouseY = float(app::AppBasic::get()->getMousePos().y)/app::getWindowHeight();
